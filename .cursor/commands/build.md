...
1. **Script Execution & Context Loading**: Run `.buildforce/scripts/bash/get-spec-paths.sh --json` FROM CURRENT WORKING DIRECTORY AND NEVER FROM SOMEWHERE ELSE!. **NEVER proceed** if script fails - display the error message to the user, explain that the `.buildforce` directory was not found, suggest: 1) check if you're in the buildforce root directory (where you ran `buildforce init`), 2) run `buildforce init .` if needed. Parse the JSON response to extract **SPEC_DIR** (absolute path). Load {SPEC_DIR}/spec.yml and {SPEC_DIR}/plan.yml into context. **Load {SPEC_DIR}/research.yml if it exists** - this provides critical implementation context including:
   ...
   If research.yml exists, use it to inform implementation decisions - it contains valuable context that may not be fully captured in spec.yaml or plan.yaml. **NEVER proceed** without spec.yml and plan.yml loaded (research.yml is optional but recommended).
