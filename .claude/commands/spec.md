...
1. **Determine create vs update mode**: Read and follow the pattern described in `templates/shared/create-update-pattern.md` from repo root.
   ...
   - Priority 2: Run `.buildforce/scripts/bash/create-spec-files.sh --json "$ARGUMENTS"` and parse JSON output for FOLDER_NAME, SPEC_FILE, SPEC_DIR, FEATURE_NUM, and IS_UPDATE flag
