...
1. **Script Execution & Context Loading**: Run `.buildforce/scripts/bash/get-spec-paths.sh --json` from repo root. Parse JSON response to extract **SPEC_DIR**. Load {SPEC_DIR}/spec.yaml into context. **NEVER proceed** without spec.yaml loaded. Check if {SPEC_DIR}/plan.yaml exists and has content beyond template to determine if this is first plan or iteration.
